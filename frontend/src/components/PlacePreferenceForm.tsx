...
export default function PlacePreferenceForm({
  eventId,
  timeRangeFrom,
  timeRangeTo,
  onClose,
  ...
  const handleIntervalChange = (
    ...
    if (newDate < rangeStart || newDate > rangeEnd) {
      notify.warning(
        `Selected ${field} time must be between ${toLocalDateTimeString(timeRangeFrom)} and ${toLocalDateTimeString(timeRangeTo)}`,
      );
      return;
    }
  ...
  const validateIntervals = () => {
    for (let i = 0; i < intervals.length; i++) {
      ...
      if (fromTime >= toTime) {
        notify.warning(
          `Interval ${i + 1}: Time "from" must be smaller than "to"`,
        );
        return false;
      }
    }
          ...
          {intervals.map((int, idx) => (
            <div key={idx} className="flex gap-2 mb-2">
              <input
                ...
                onChange={(e) =>
                  handleIntervalChange(idx, "from", e.target.value)
                }
              ...
              <input
                ...
                onChange={(e) =>
                  handleIntervalChange(idx, "to", e.target.value)
                }
                className="border p-1"
              />
              {intervals.length > 1 && (
