...
export default function GenericVotingForm({ eventId, title, voteType, filterOptions, providedOptions }: GenericVotingFormProps) {
    ...
    const handleToggle = (optionId: string) => {
        if (!isMultiSelect) { // for radio
            setSelected([optionId]);
        } else {
            setSelected(prev => { // for checkbox
    ...
    const handleVote = async () => {
        if (selected.length === 0) return notify.warning("Please select an one option.");
    ...
    if (filteredOptions.length === 0) {
        return (
            <div className="mt-8 p-6 bg-gray-50 rounded-lg text-center border dark:bg-gray-800 dark:border-gray-700">
                <p className="text-gray-600 dark:text-gray-400">No options available for {title}.</p>
            </div>
        );
    }
                            ...
                            <label className="flex ml-4 mr-4 items-center gap-8 cursor-pointer">
                                <div className="flex-1">
                                    ...
                                    <p className="text-sm text-gray-700 dark:text-gray-100">
                                        📅 {new Date(opt.timeFrom).toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'short' })} 
                                        {' – '}
                                        {new Date(opt.timeTo).toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'short' })}
                                    </p>
