...
export default function CommentSection({ eventId }: Props) {
    ...
    const handleEdit = async (commentId: string) => {
        ...
        try {
            const response = await axios.put(`/events/${eventId}/comments/${commentId}`, {
                content: editingContent.trim()
            });
            setComments(prev => prev.map(comment => 
                comment.id === commentId ? { ...comment, content: editingContent.trim(), isEdited: true } : comment
            ));
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex justify-between items-center mt-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-lg font-medium transition-colors disabled:cursor-not-allowed"
                            >
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {comments.map((comment) => (
                        <div
                            key={comment.id}
                            className="flex gap-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                        >
                                ...
                                {editingCommentId === comment.id ? (
                                    <div className="mt-2">
                                        <textarea
                                            ...
                                            maxLength={2000}
                                            rows={3}
                                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 resize-none"
                                        />
                                        <div className="flex gap-2 mt-2">
                                            ...
                                            <button
                                                ...
                                                className="flex items-center gap-1 px-3 py-1.5 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200 rounded text-sm"
                                            >
                                                <X className="w-4 h-4" />
                                                Cancel
                                            </button>
