...
import { Trash2, Filter, AlertTriangle, RefreshCw, Calendar } from "lucide-react";
...
export default function DataCleanupPage() {
    ...
    const loadEvents = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`/admin/events/cleanup?daysOld=${daysThreshold}`);
            setEvents(res.data);
        } catch (err) {
            console.error(err);
    ...
    const handleDeleteSingle = async (id: string) => {
        if (!window.confirm("Are you sure you want to delete this specific event?")) return;
        try {
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="mb-8">
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-400">
                    Manage system storage by removing outdated events. Be careful, these actions are irreversible.
                </p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
                    <h3 className="font-semibold text-gray-900 dark:text-white">Found Events ({events.length})</h3>
                    <button onClick={loadEvents} title="Refresh" className="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors">
